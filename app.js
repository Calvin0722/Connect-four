addEventListener('DOMContentLoaded', () => {
    ...
    const displayCurrPlayer = document.querySelector("#current-player")
    let won = false

    let curentPlayer  = 1

    // Add enent listener to all the squares to take the squares
        ...
        squares[i].onclick = () => {
            ...
            if (!won) {
                if (squares[index + 7].classList.contains("taken") && 
                    ...
                    if (curentPlayer === 1) { // Player one takes the square
                        squares[index].classList.add('taken')
                        squares[index].classList.add('player-one')
                        curentPlayer = 2;
                        displayCurrPlayer.innerHTML = curentPlayer
                    } else { // Player two takes the square
                        squares[index].classList.add('taken')
                    ...
                    // The player can only go the a square that has a square below it.
